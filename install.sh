...
if [ -z "$USER_ID" ]; then
    read -p "Enter your User ID (Found on dashboard): " USER_ID
    if [ -z "$USER_ID" ]; then
...
if [ -z "$PORT" ]; then
    read -p "Enter Local Port to tunnel [80]: " PORT
    PORT=${PORT:-80}
fi
...
[Service]
Type=simple
ExecStart=$INSTALL_DIR/$BINARY_NAME -url $SIGNAL_URL -user $USER_ID -id $DEVICE_ID -port $PORT
Restart=always
RestartSec=10
